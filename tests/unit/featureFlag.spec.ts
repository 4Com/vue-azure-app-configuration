...
function setupAzureAppConfigClient(
  returnEnabledFeatureFlag = true,
  ...
  Vue.prototype.$azureAppConfig.client = {
    getConfigurationSetting: async function ({ key, label }) {
      ...
      let value: string = `{"id":"${key.replace(
        featureFlagPrefix,
        ""
      )}","enabled":${returnEnabledFeatureFlag},"conditions":{"client_filters":[`;
      if (includeClientFilters)
        value +=
          '{"name":"Microsoft.Targeting","parameters":{"Audience":{"Users":["me@example.com"]}}}';
      value += "]}}";
      return {
...
describe("featureFlag.js", () => {
  it("getFeatureFlagAsync() returns feature flag with default label", async () => {
    expect(
      await wrapper.vm.$azureAppConfig.getFeatureFlagAsync(testFeatureFlagName)
    ).toStrictEqual({
      contentType: featureFlagContentType,
  ...
  it("getFeatureFlagAsync() returns feature flag with overridden label", async () => {
    expect(
      await wrapper.vm.$azureAppConfig.getFeatureFlagAsync(
        testFeatureFlagName,
        overriddenFeatureFlagLabel
      )
    ).toStrictEqual({
      contentType: featureFlagContentType,
      key: `${featureFlagPrefix}${testFeatureFlagName}`,
  ...
  it("isFeatureFlagEnabledAsync() returns true when feature flag exists and is enabled", async () => {
    expect(
      await wrapper.vm.$azureAppConfig.isFeatureFlagEnabledAsync(
        testFeatureFlagName
      )
    ).toBe(true);
  });
  ...
  it("isFeatureFlagEnabledForUserAsync() returns true when feature flag exists and is enabled", async () => {
    expect(
      await wrapper.vm.$azureAppConfig.isFeatureFlagEnabledForUserAsync(
        testFeatureFlagName
      )
    ).toBe(true);
  });
